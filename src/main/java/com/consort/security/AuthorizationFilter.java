...
public class AuthorizationFilter extends SecurityFilter {
    ...
    private static Config createSecurityConfig(final String authorizerName, final String roleName) {
        ...
        final String jwk_kids = EnvironmentContext.getInstance().getenv(EnvironmentProps.JwkKid);
        ...
        for (final String kid : jwk_kids.split(",")) {
            ...
            try {
                final URL urlToJWK = new URL(EnvironmentContext.getInstance().getenv(EnvironmentProps.JwkUrl));
                final JwkProvider provider = new UrlJwkProvider(urlToJWK);
                final Jwk jwk = provider.get(kid);
                if (jwk != null && EnvironmentContext.getInstance().getenv(EnvironmentProps.JwkAlg)
                        .equalsIgnoreCase(jwk.getAlgorithm())) {
        ...
        if (!StringUtils.isBlank(authorizerName) && !StringUtils.isBlank(roleName)) {
            config.addAuthorizer(authorizerName, new DomainModelServiceAttributeAuthorizer(AUTHORIZER_SCOPE, roleName));
        }
