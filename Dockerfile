...
ENTRYPOINT ["/bin/sh", "-c"]
CMD ["java -Xmx128m -server -jar /microservice/domain-model-service.jar -Duser.timezone=UTC"]

COPY --from=builder /microservice/target/domain-model-service-*with-depen*.jar /microservice/domain-model-service.jar